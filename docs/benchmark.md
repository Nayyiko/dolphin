...
| 指标 | 目标值 | 实际值（2026-08-16 实测，3 轮取优） |
|------|--------|----------------|
| 网关 QPS | 10,000+ | 55453 |
| P99 延迟 | < 20ms | 11.05 ms |
| P95 延迟 | < 10ms | 6.96 ms |
| 错误率 | < 0.1% | 0.00% |

> 注：裸吞吐用 `configs/gateway_bench.yaml`（关限流）+ `bin/loadgen.exe`（连接复用修复版）实测。
> 早期"2093 QPS"无原始报告，且 loadgen 曾因不读响应体而无法复用连接，每请求新建 TCP 连接，
> 高并发下触发 Windows 临时端口池耗尽，QPS 被严重低估（详见 `docs/evidence-plan.md` 证据 4a++）。
