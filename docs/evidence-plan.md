...
**证据 4a++（排障故事）：压测工具连接不复用，QPS 被低估 26 倍**
- 背景：裸网关吞吐复跑（关限流、100 并发）连续 3 轮 30~99% 连接错误，单条 curl 探活却正常。起初怀疑机器负载，关闭全部窗口后仍失败。
- 排查：错误全部是快速传输层错误（最大延迟 <3s，无超时）；且 P50 ≈48ms——每请求约一次握手的开销。顺着"每个请求都在建新连接"的线索，检查 loadgen 源码发现：`resp.Body.Close()` 在**未读响应体**的情况下直接关闭——Go 的 http.Transport 只有读完 body 才能复用连接，于是每个请求新建一条 TCP 连接。
- 根因：30s × 100 并发 ≈ 2 万次新连接，超出 Windows 临时端口池（49152–65535，约 1.6 万个）→ 端口耗尽 + TIME_WAIT → 连接拒绝。这也解释了为什么此前的测试"碰巧干净"：500 QPS×15s≈7500 次连接，未触顶端口池。
- 修复：loadgen 关闭前 `io.Copy(io.Discard, resp.Body)` 读完响应体，连接复用。修复前后对比：同一台机器、同一网关，739 QPS/32% 连接错误 → **55,453 QPS / 0 连接错误 / P99 11.05ms**（3 轮一致）。
- **面试讲**："测压测工具本身也会污染结果。loadgen 不读响应体导致 keep-alive 失效，每请求一条新连接，高并发下把 Windows 端口池打满——我以为网关只能跑 700 QPS，实际是 5.5 万 QPS。这也让我对'先怀疑观测手段，再怀疑系统'有了实操体会。"
...
| — | 裸网关吞吐 | ✅ 已有（2026-08-16 复跑，连接复用修复后） | 55,453 QPS / P95 6.96ms / P99 11.05ms / 0 错误（3 轮取优） |
...
| 网关能力 | 裸吞吐 55,453 QPS / P99 11.05ms / 0 错误（3 轮一致） | 连接复用修复后实测，路由/中间件层性能 |
